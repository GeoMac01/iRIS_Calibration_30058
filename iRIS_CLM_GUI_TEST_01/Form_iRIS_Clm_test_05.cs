...
namespace iRIS_CLM_GUI_TEST_05
{
    public partial class Form_iRIS_Clm_test_05 : Form
    {
        ...
        //======================================================================
        private async Task<bool> RampDACLI(double startRp, double stopRp, double stepRp, bool rdIntADC)//external PCON
        {
            arrIndex = 0;
            bool rtnRamp = false;

            if (startRp < stopRp) //non inverted ramp 0V-5V
            {
                for (double startRpLp = startRp; (startRpLp <= stopRp) && (stopLoop == false); startRpLp = startRpLp + stepRp)
                {
                    WriteDAC(startRpLp, 0);//update Pcon DAC

                    rtnRamp = await ReadAllanlg(rdIntADC); //display data return good or error 
                    if (rtnRamp == false) { break; }

                    bool ldArr = await LoadArray(arrIndex);
                    arrIndex++;
                }
            }

            else if (startRp > stopRp) //inverted ramp 5V-0V
            {
                for (double startRpLp = startRp; (startRpLp >= stopRp) && (stopLoop == false); startRpLp = startRpLp - stepRp)
                {
                    WriteDAC(startRpLp, 0);//update Pcon DAC

                    rtnRamp = await ReadAllanlg(rdIntADC); //displays current in bits and errors
                    if (rtnRamp == false) { break; }
        ...
        private async Task<bool> RampDAC1(double startRp, double stopRp, double stepRp, bool rdIntADC)//external PCON  can be replaced....
        {
            ...
            if (startRp < stopRp) {
                ...
                for (double startRpLp = startRp; (startRpLp <= stopRp) && (stopLoop == false); startRpLp = startRpLp + stepRp)
                {
                    ...
                    rampState = await ReadAllanlg(rdIntADC);//displays current in bits
        ...
        */
        ...
        private async Task<bool> ReadAllanlg(bool fullRd) {//reads all data
            ...
            if (pm100ok == true) //PM100 Connected
            {
                ...
                if (pwrRead > maxPw)
                {
                    ...
                    if (ChkBx_AnlgModSet.Checked == true) { WriteDAC(5.000, 0); }//inverted PCON 5V/0V
                    else WriteDAC(0, 0);
            ...
            return true; //above break will return false if error...
        }
        //======================================================================
        ...
        private async Task<bool> CalVGA_0_5V()
        {
            if (Bt_CalVGA.BackColor == Color.Coral)
            {
                ...
                Tb_VPcon.Text = startRp.ToString("00.000");
                WriteDAC(startRp, 0);//0V
                ...
                if ((Convert.ToBoolean(Read_USB_Digit_in(2)) == true) &&  (stopLoop == false)) //Laser OK //test
                {
                    ...
                    for (int i = 0; (i <= 2)&&(stopLoop == false); i++)//3 VGA set iteration //test
                    {
                        ...
                        if (boolCalVGA1 == false)//test for error
                        {
                            stopLoop = true;
                            break;//stop test
                        }
                        else
                        {
                            for (vgaVal = 20; (vgaVal <= 80)&&(stopLoop == false); vgaVal++)//Ramp and set VGA
                            {
                                ...
                                else
                                {
                                    ...
                                    boolCalVGA1 = await SendToSerial(CmdSetVgaGain, Tb_VGASet.Text, 300, 9);//send vga value
                            ...
                            WriteDAC(thresholdOffsetAdj, 0); //set to 0.5V PCON with above VGA
        ...
        //======================================================================
        private async Task<bool> CalVGA_5V_0()//Inverted External PCON 5V to 0V
        {
            if (Bt_CalVGA.BackColor == Color.Coral)
            {
                Prg_Bar01.Maximum = 120;
                ...
                const double thresholdOffsetAdj = 4.500;
                const double startRp = 5.000;
                const double stepRp = 0.020;//value 0.01..0.05..
                double onePerctPower = (Convert.ToDouble(Tb_NomPw.Text))/1000;
                double calPower = 0;
                double setOffSet = 0;
                ...
                string Pw_Pcon_MaxPw = string.Empty;//0V
                string Pw_Pcon_MinPw = string.Empty;//4.45V
                string Pw_Pcon_Thrs = string.Empty; //power at threshold voltage 0.5V or 4.50V
                string Pw_Thres_PCON = string.Empty;//voltage registered at 0.1% power 

                string Pw_EnOff = string.Empty;// 0 power
                string Pw_450vPCON = string.Empty;// laser threshold
                string offset = string.Empty;
                bool initvga = false;
                bool boolCalVGA1 = false;
                bool goodOffset = false;
                ...
                if ((Convert.ToBoolean(Read_USB_Digit_in(2)) == true) && (stopLoop == false)) //Laser OK //test
                {
                    ...
                    for (int i = 0; (i <= 2) && (stopLoop == false); i++)//3 VGA set iteration //test
                    {
                        boolCalVGA1 = await RampDAC1(startRp, stopRp, stepRp, false); //5V to 0V set to MAX power
                        ...
                        else
                        {
                            for (vgaVal = 20; (vgaVal <= 80) && (stopLoop == false); vgaVal++)//Ramp and set VGA
                            {
                                ...
                                {
                                    Tb_VGASet.Text = vgaVal.ToString("0000");//set VGA
                                    ...
                                    {
                                        double pm100Res = Convert.ToDouble(Lbl_PM100rd.Text);//mW
                                        if (pm100Res > setPower) break; // added 11/04/2018 the power should be a least above min max power
                                    }
                                }
                                Prg_Bar01.Increment(10);
                            }

                            WriteDAC(thresholdOffsetAdj, 0); //set to 4.5V PCON with above VGA to adjust offset for min power
                            await Task.Delay(200);
                            ...
                            for (int j = 0; (j < 59) && (stopLoop == false); j++) //adjust V offset
                            {
                                ...
                                if (boolCalVGA1 == true)
                                {
                                    ...
                                    if (calPower > (setPower * 0.00130)) { setOffSet = setOffSet + 0.002; } //add offset...reduces power
                                    else if (calPower < (setPower * 0.00070)) { setOffSet = setOffSet - 0.002; } //reduce offset increase power
                                    else { goodOffset = true; }
                ...
                {
                    Prg_Bar01.Maximum = 120;
                    WriteDAC(stopRp, 0);//0V
                    await Task.Delay(300);
                    initvga = await ReadAllanlg(false);
                    Pw_Pcon_0V = Lbl_PM100rd.Text;//maximum power
                    Prg_Bar01.Increment(10);

                    WriteDAC(pwThresholdVolt, 0);//4.450V
                    await Task.Delay(300);
                    initvga = await ReadAllanlg(false);
                    Pw_Pcon_445V = Lbl_PM100rd.Text;
                    Prg_Bar01.Increment(10);
                    ...
                    WriteDAC(startRp, 0);//reset test and stop
                    Set_USB_Digit_Out(0, 0); //Laser Disable
                    await Task.Delay(300);
                    initvga = await ReadAllanlg(false);
                    Pw_EnOff = Lbl_PM100rd.Text;
                    ...
                    #endregion set final test
                    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//
                    #region set file and record TXT results
                    try
                    {
                        if (File.Exists(filePathRep))
                        {
        ...
        private async Task<bool> PD_Calibration() {
            ...
            pdCalTask = await SendToSerial(CmdTestMode, StrEnable, 300, 9);             //Test mode
            pdCalTask = await SendToSerial(CmdsetTTL, StrEnable, 300, 9);               //invert TTL line
            pdCalTask = await SendToSerial(CmdEnablLogicvIn, StrDisable, 300, 9);       //Non Invert Enable line
            pdCalTask = await SendToSerial(CmdSetInOutPwCtrl, StrDisable, 300, 9);      //set internal power

            Set_USB_Digit_Out(0, 1);                                                    //Enable laser  
            pdCalTask = await SendToSerial(CmdLaserEnable, StrEnable, 300, 9);          // 

            pdCalTask = await RampDACLI(startRp, stopRp, stepRp, true);          //Log ON, external PCON
        ...
        private async Task<bool> LsFinalSet() //load data in laser
        {
            if (Bt_FinalLsSetup.BackColor == Color.Coral)
            {
                ...
                if (ChkBx_IntPwCtrl.Checked == true) { chkBxStateExtPwCtrl = StrEnable; }
        ...
        private async Task<bool> WriteResToDb(sbyte saveData)
        {
            ...
            try
            {
                ...
                if (saveData == 1) {
                    ...
                    //************************************************************************//
                    if (ChkBx_IntPwCtrl.Checked == true) { cmd.Parameters.AddWithValue("@val28", "Internal"); }
            ...
            if (Convert.ToBoolean(Read_USB_Digit_in(2)) == true) //Laser OK 
                {
                    ...
                    setPwCheck = await SetConfigIO();
                    
                    if (ChkBx_DigitModSet.Enabled == true)
                    {
                        Set_USB_Digit_Out(1, 0);//digital modulation line inverted
                    }
                    else if (ChkBx_DigitModSet.Enabled == false)
                    {
                        Set_USB_Digit_Out(1, 1);//digital modulation line non inverted
                    }

                    if (ChkBx_EnableSet.Checked == true)
                    {
                        Set_USB_Digit_Out(0, 0); //Laser Enable inverted
                    }
                    else if (ChkBx_EnableSet.Checked == false)
                    {
                        Set_USB_Digit_Out(0, 1);//Laser enable non inverted
                    }

                    if (ChkBx_IntPwCtrl.Checked == true) //internal PCON - Calibrated now, then the power setting in 1/10mW should be OK
                    {
                        ...
                        double finalPowerDbl = Convert.ToDouble(Tb_NomPw.Text) * 10;  //nominal power from database
                    ...
                    else if (ChkBx_IntPwCtrl.Checked == false) //external PCON 0V or 5V -- 
                    {
                        ...
                        if (ChkBx_AnlgModSet.Checked == false) //non inverted 
                        {
                            //dacPCONSet = 5.000;
                            dacPCONSet = 4.900;
                            setPwCheck = await SendToSerial(CmdAnalgInpt, StrDisable, 300, 9); //Non Inv. PCON
                        }
                        else if (ChkBx_AnlgModSet.Checked == true) //inverted 
                        {
                            dacPCONSet = 0.000;
                            setPwCheck = await SendToSerial(CmdAnalgInpt, StrEnable, 300, 9); //Inv. PCON
                        }
                    ...
                    setPwCheck = await SendToSerial(CmdLaserEnable, StrEnable, 300, 9);//Laser Enable and stays on
                    await Task.Delay(100);
        ...
        //======================================================================
        private async Task<bool> SetConfigIO()
        {
            ...
            double finalPowerDbl = Convert.ToDouble(Tb_NomPw.Text) * 10;
            string finalPower = finalPowerDbl.ToString("0000");
            ...
            bool finalSet1 = await SendToSerial(CmdTestMode, StrEnable, 300, 9);
            finalSet1 = await SendToSerial(CmdSetInOutPwCtrl, chkBxStateExtPwCtrl, 300, 9);
            finalSet1 = await SendToSerial(CmdEnablLogicvIn, chkBxStateEnblSet, 300, 9);
            finalSet1 = await SendToSerial(CmdsetTTL, chkBxStateDigitModSet, 300, 9);
            finalSet1 = await SendToSerial(CmdAnalgInpt, chkBxStateAnlgModSet, 300, 9);
            finalSet1 = await SendToSerial(CmdTestMode, StrDisable, 300, 9);

            finalSet1 = await SendToSerial(CmdSetLsPw, finalPower, 300, 9);
            ...
            try
            {
                ...
                while (rdr.Read())
                {
                    ...
                    if (readstuff.Contains(Tb_LaserPN.Text)) {
                        ...
                        if ((rdr["PowerControlSource"].ToString()) == "Internal  ") { ChkBx_IntPwCtrl.Checked = true; }
                        else { ChkBx_IntPwCtrl.Checked = false; }
            ...
            try
            {
                ...
                while (rdr.Read())
                {
                    ...
                    if (readstuff.Contains(Tb_LaserSerNb.Text)) { //serial number found://issue with multiple entries!!!!!!
                        ...
                        Tb_Wavelength.Text = rdr["Diode_Wavelength"].ToString().PadLeft(4, '0').TrimStart('0');//load test wavelength
